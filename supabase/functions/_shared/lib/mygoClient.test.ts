...
// Test 11: Empty response should throw error
Deno.test("parseListCityResponse should throw error on empty XML response", () => {
  ...
  assertThrows(
    ...
    Error,
    "No <City> elements found",
  );
});

// Test 12: Response without City elements should throw error
Deno.test("parseListCityResponse should throw error when no City elements present", () => {
...
// Test 13: Handle XML wrapped in Root without explicit ListCity tag
Deno.test("parseListCityResponse should parse XML wrapped in Root without ListCity tag", () => {
  const xmlWithoutListCity = '<?xml version="1.0" encoding="utf-8"?>' +
    '<Root>' +
    '<City><Id>1</Id><Name>Tunis</Name><Region>Nord</Region></City>' +
    '<City><Id>2</Id><Name>Sousse</Name></City>' +
    '</Root>';
  
  const cities = parseListCityResponse(xmlWithoutListCity);
  assertEquals(cities.length, 2);
  assertEquals(cities[0].id, 1);
  assertEquals(cities[0].name, "Tunis");
  assertEquals(cities[0].region, "Nord");
